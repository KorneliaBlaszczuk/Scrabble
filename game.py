...
WIN = pygame.display.set_mode((WIDTH, HEIGHT + 2 * EXTRA_SPACE))
...
def name_win():
    ...
    input_rect = pygame.Rect(rect_x, 2 * HEIGHT // 3, rect_width, rect_height)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_e:
                    end(player_name, player_score, bot_score)
                    run = False
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if len(current_next_pos) == 2:
                    ...
                    rack_x, rack_y = board.row_col_to_coord(rack_row, rack_col)
...
def end(player_name, player_score, bot_score):
    WIN.fill(CENTRAL_COLOR)
    pygame.font.init()
